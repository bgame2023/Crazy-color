...
public class GameController : MonoBehaviour 
{
    ...
    [SerializeField] GameObject[] NextBalls;
	...
	public void GameOver()
	{
		...
		{
			int bestScore = PlayerPrefs.GetInt(Constants.key_best_medium, 0);
			if (score > bestScore)
			{
				bestScore = score;
				PlayerPrefs.SetInt(Constants.key_best_medium, bestScore);
			}
		}
		if (level == Level.HARD)
		{
			int bestScore = PlayerPrefs.GetInt(Constants.key_best_hard, 0);
			if (score > bestScore)
			{
				bestScore = score;
				PlayerPrefs.SetInt(Constants.key_best_hard, bestScore);
			}
		}
	}
	...
	public void Quit()
	{
		SoundManager.GetInstance().PlayClickSound();
        Time.timeScale = 1f;
        Application.LoadLevel(Constants.sceneMain);
	}
